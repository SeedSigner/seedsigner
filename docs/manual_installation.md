...
Best practice is to first setup a fresh, single purpose laptop. To do this, it is best to walk into a big box store and purchase a cheap laptop with cash anonymously.  Go home, and wipe this laptop with a fresh install of Ubuntu. Follow this tutorial if you need any help (https://ubuntu.com/tutorials/install-ubuntu-desktop#1-overview). After setting up the laptop, only use it from now on for SeedSigner!
