...
def test_get_standard_derivation_path():
    ...
    for args, expected in vectors_args_expected.items():
        ...
        if type(expected) is str:
            ...
            a_dict = {}
            if len(args) == 1:
                a_dict = {"network": args[0]}
            elif len(args) == 2:
                a_dict = {"network": args[0], "wallet_type": args[1]}
            elif len(args) == 3:
                a_dict = {
                    "network": args[0],
                    "wallet_type": args[1],
...
def test_get_xpub():
    ...
    vectors_args_expected = {
        ...
        (
            vector_seeds[1],
            "m/0/2147483647'/1/2147483646h",
            "main",
        ): "xpub6ERApfZwUNrhLCkDtcHTcxd75RbzS1ed54G1LkBUHQVHQKqhMkhgbmJbZRkrgZw4koxb5JaHWkY4ALHY2grBGRjaDMzQLcgJvLJuZZvRcEL",
        (
            vector_seeds[1],
            "m/0/2147483647h/1/2147483646'/2",
            "main",
        ...
        (vector_seeds[4], "m/84'/0'/0'", "main"): bip32.HDKey.from_string(
            "zpub6rFR7y4Q2AijBEqTUquhVz398htDFrtymD9xYYfG1m4wAcvPhXNfE3EfH1r1ADqtfSdVCToUG868RvUUkgDKf31mGDtKsAYz2oz2AGutZYs"
        ).to_base58(version=b"\x04\x88\xb2\x1e"),
        # https://github.com/satoshilabs/slips/blob/master/slip-0132.md#bitcoin-test-vectors
        (vector_seeds[4], "m/49'/0'/0'", "main"): bip32.HDKey.from_string(
            "ypub6Ww3ibxVfGzLrAH1PNcjyAWenMTbbAosGNB6VvmSEgytSER9azLDWCxoJwW7Ke7icmizBMXrzBx9979FfaHxHcrArf3zbeJJJUZPf663zsP"
        ).to_base58(version=b"\x04\x88\xb2\x1e"),
        # https://github.com/bitcoin/bips/blob/master/bip-0049.mediawiki#test-vectors
        (vector_seeds[4], "m/49'/1'/0'", "test"): bip32.HDKey.from_string(
            "upub5EFU65HtV5TeiSHmZZm7FUffBGy8UKeqp7vw43jYbvZPpoVsgU93oac7Wk3u6moKegAEWtGNF8DehrnHtv21XXEMYRUocHqguyjknFHYfgY"
        ).to_base58(version=b"\x04\x35\x87\xcf"),
        # https://github.com/satoshilabs/slips/blob/master/slip-0132.md#bitcoin-test-vectors
        (vector_seeds[4], "m/44'/0'/0'", "main"): bip32.HDKey.from_string(
    ...
    for args, expected in vectors_args_expected.items():
        ...
        # call with named params
        print(
            f'  {func.__name__}(seed_bytes={args[0]}, derivation_path="{args[1]}", embit_network="{args[2]}") == "{expected}"'
        )
        assert (
            str(
                func(seed_bytes=args[0], derivation_path=args[1], embit_network=args[2])
            )
            == expected
        )
...
def test_get_single_sig_address():
    ...
    for args, expected in vectors_args_expected.items():
        ...
        # call with ordered params
        print(
            f'  {func.__name__}(HDKey.from_string("{args[0]}"), *{args[1:5]}) == "{expected}"'
        )
        assert str(func(*args)) == expected

        # call with named params
        print(
            f'  {func.__name__}(xpub=HDKey.from_string("{args[0]}"), script_type="{args[1]}", index={args[2]}, is_change={args[3]}, embit_network="{args[4]}") == "{expected}"'
        )
        assert (
            str(
                func(
...
def test_get_multisig_address():
    ...
    vector_args_expected = {
        ...
        (
            ...
            False,
            "test",
        ): "tb1q7tpecll8jhp77yqdeyt2t8q5swxmmqeh2v22cqpms5dxlp6p27dqlftet8",
        (
            "wsh(sortedmulti(2,[8d55ff0d/48h/1h/0h/2h]tpubDDxNVWk924RTUhdkVB2uLHw1hGMPNMGufpZefhkkswjbZppVZcuMdjYKQN4ewUog9vbL6RBLFPRWcgTGT7kYP79N6thyJ43ELUs4N2szXMg/{0,1}/*,[73c5da0a/48h/1h/0h/2h]tpubDFH9dgzveyD8zTbPUFuLrGmCydNvxehyNdUXKJAQN8x4aZ4j6UZqGfnqFrD4NqyaTVGKbvEW54tsvPTK2UoSbCC1PJY8iCNiwTL3RWZEheQ/{0,1}/*,[0be174ee/48h/1h/0h/2h]tpubDEsePyLPkbxbrDiZSTTWdsviiNtiQjrvvzZnkLtG72QYLBygEsXePRsTdXi8DeMA7taCuuvoEBjUAfFrsNZeQJqfvG9fFoujYWbFPYUn7ux/{0,1}/*))#zw6cnrlk",
            0,
            True,
            "test",
        ): "tb1q7h94ywhfjrpxdfzwl4dcawrg80r4rywswjrh447x4n3e5t3m0jms9jh7pm",
        ...
        (
            ...
            False,
            "test",
        ): "2MtgJH28mZWNWU7VRU4ba6ciFbRRGYWZDt3",
        (
            "sh(wsh(sortedmulti(2,[73c5da0a/48h/1h/1h/0h/1h]tpubDFH9dgzveyD8yHQb8VrpG8FYAuwcLMHMje2CCcbBo1FpaGzYVtJeYYxcYgRqSTta5utUFts8nPPHs9C2bqoxrey5jia6Dwf9mpwrPq7YvcJ/{0,1}/*,[0be174ee/48h/1h/0h/1h]tpubDEsePyLPkbxbnj6XuKvWwdERHaKkikZxaGJ9sJqmM7okbZXgkNSFiGU6GX6qEes6kD8f9Z9FosYB9UEnBSgBEyEwwJhj4uUcFE1WE8VtKoh/{0,1}/*,[8d55ff0d/48h/1h/0h/1h]tpubDDxNVWk924RTT3vyGLHdSDoZ2JUVX7jUsPcwCQ9MrKHAtJrW5zECTF9rFHCvqu526E4PjHp61hBknts2c5aGexvX7hvCZ8TGPvQFdzxxy59/{0,1}/*)))#2ujlfp73",
            0,
            True,
            "test",
        ): "2NAjjwUQqwD9XRGLeQ6TitSUyMHUz3cLiWm",
        ...
        (
            ...
            False,
            "test",
        ): "2NBXci43Y2fagvrFYTg3QmXj2LCPU2oaRFH",
        (
            "sh(sortedmulti(2,[8d55ff0d/45h]tpubDANogJ2yfnizHwX7fSi5kUVzybyuPXDhgHB2TR9TUvkSLZFW73cRq4STKFDpx7qjJJiisyq82tbu4CeiYtmKEmT1xoCq9P8BPvXV31HUh6d/{0,1}/*,[0be174ee/45h]tpubDBkeVF2tDNT1Pz7L47iJeBB6RokU12LX6x4E6Ph8T89hmjQfB77q1AMyGwL8qpREVGq9sCJEbWwmnemwNTxnpxGn1di7BGy8jx9wEi5Vahu/{0,1}/*,[73c5da0a/45h]tpubDBKsGC1UqBDNvx9aivFmxZNgeZTUnmsCFGhWrqkLzucUCDePvbWWm3n8tAaAwMmxBG2ihdKCG9fzBdUnMxKx5PrkiqSZFi6Vkv6msUs9ddN/{0,1}/*))#p5t8sa8c",
            0,
            True,
            "test",
        ): "2MuWQTq7hUGiX1HpXuPRnf7YTM42H5zoEwj",
    ...
    for args, expected in vector_args_expected.items():
        ...
        if type(expected) == str:
            ...
            if args[1:4] == (0, False, "main"):
                print(
                    f'  {func.__name__}(Descriptor.from_string("{descriptor}")) == "{expected}"'
                )
                assert func(descriptor) == expected

            # call with ordered params
            print(
                f'  {func.__name__}(Descriptor.from_string("{descriptor}"), *{args[1:4]}) == "{expected}"'
            )
            assert func(descriptor, *args[1:4]) == expected

            # call with named params
            print(
                f'  {func.__name__}(descriptor=Descriptor.from_string("{descriptor}"), index={args[1]}, is_change={args[2]}, embit_network="{args[3]}") == "{expected}"'
            )
            assert (
                func(
        ...
        # test exceptions
            ...
            with pytest.raises(expected):
                print(
                    f'  {func.__name__}(Descriptor.from_string("{descriptor}"), *{args[1:4]}) raises Exception'
                )
            ...
            with pytest.raises(expected):
                print(
                    f'  {func.__name__}(descriptor=Descriptor.from_string("{descriptor}"), index={args[1]}, is_change={args[2]}, embit_network="{args[3]}") raises Exception"'
                )
                func(
                    descriptor=descriptor,
                    index=args[1],
...
def test_parse_derivation_path():
    ...
    vectors_args = {
        ...
        (
            ...
            False,
            5,
        ): "m/123'/9083270/9083270/9083270/9083270/0/5",
