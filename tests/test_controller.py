...
class TestController(BaseTest):
    ...
    def test_missing_settings_get_defaults(self):
        ...
        # Settings defaults
        assert (
            controller.settings.get_value(SettingsConstants.SETTING__LOCALE)
        ...
        ) == [i for i, j in SettingsConstants.ALL_COORDINATORS if i != "kpr"]
        assert (
            controller.settings.get_value(SettingsConstants.SETTING__BTC_DENOMINATION)
            == SettingsConstants.BTC_DENOMINATION__THRESHOLD
        )

        # Advanced Settings defaults
        assert (
            controller.settings.get_value(SettingsConstants.SETTING__NETWORK)
        ...
        assert (
            controller.settings.get_value(SettingsConstants.SETTING__XPUB_EXPORT)
            == SettingsConstants.OPTION__ENABLED
        )
        assert controller.settings.get_value(SettingsConstants.SETTING__SIG_TYPES) == [
            i for i, j in SettingsConstants.ALL_SIG_TYPES
        ]
        assert controller.settings.get_value(
        ...
        assert (
            controller.settings.get_value(SettingsConstants.SETTING__XPUB_DETAILS)
            == SettingsConstants.OPTION__ENABLED
        )
        assert (
        ...
        assert (
            controller.settings.get_value(SettingsConstants.SETTING__BIP85_CHILD_SEEDS)
            == SettingsConstants.OPTION__DISABLED
        )
        assert (
            controller.settings.get_value(SettingsConstants.SETTING__MESSAGE_SIGNING)
            == SettingsConstants.OPTION__DISABLED
        )
        assert (
        ...
        assert (
            controller.settings.get_value(SettingsConstants.SETTING__DIRE_WARNINGS)
            == SettingsConstants.OPTION__ENABLED
        )
        assert (
            controller.settings.get_value(SettingsConstants.SETTING__QR_BRIGHTNESS_TIPS)
            == SettingsConstants.OPTION__ENABLED
        )
        assert (
            controller.settings.get_value(SettingsConstants.SETTING__PARTNER_LOGOS)
            == SettingsConstants.OPTION__ENABLED
        )

        # Hidden Settings defaults
        assert (
            controller.settings.get_value(SettingsConstants.SETTING__QR_BRIGHTNESS)
            == 62
        )
