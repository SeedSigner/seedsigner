...
def test_p2tr_change_detection():
    ...
    assert pp.destination_addresses == [
...
def test_p2sh_legacy_multisig():
    ...
    assert psbt_parser.destination_addresses == [
        "bcrt1q8q5uk9z7ta08h8hvknysd5n80w6f7kuvk5ey2m"
    ]
    assert psbt_parser.destination_amounts == [50000]

    assert (
        psbt_parser.get_change_data(0)["address"]
        == "2NEnA5emHw9Q6vHXr912hGMSPtnrwAMReLz"
    )
    assert psbt_parser.get_change_data(0)["amount"] == 58969

    assert (
        psbt_parser.get_change_data(1)["address"]
        == "2N5eN5vUpgsLHAGzKm2VfmYyvNwXmCug5dH"
    )
...
def test_p2sh_p2wpkh_nested_segwit():
    ...
    descriptor = Descriptor.from_string(
        "sh(wpkh([c751dc07/49h/1h/0h]tpubDDS23bf7c9mdfWpuvA61HHCYDusq25UtMNYsFagKPNMNWHSm8bvwmNNP2KSpivN3gQWAK8fhDFk3dzgoBn9rPoMncKxJuqNAv7sJMShbZ6i/<0;1>/*))#7sn8gf37".replace(
            "<0;1>", "{0,1}"
        )
    )
    ...
    assert psbt_parser.destination_addresses == [
        "tb1qs7mdpjq7g7zq46vvycr8d6udc7za726ut8har9krfxpnc7kr04gqmdy2e4"
    ]
    assert psbt_parser.destination_amounts == [93000]

    assert (
        psbt_parser.get_change_data(0)["address"]
        == "2Mz3MthXyM4YDjLPw1V4PAacKt4pD8Cz8N3"
    )
...
def test_parse_op_return_content():
    ...
    assert (
        psbt_parser.op_return_data
        == "Chancellor on the brink of third bailout".encode()
    )
    ...
    assert (
        psbt_parser.spend_amount == 0
    )  # This is a self-spend; no value being spent, other than the tx fee
