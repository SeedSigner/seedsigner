...
class TestSettingsFlows(FlowTest):
    ...
    def test_multiselect(self):
        ...
        self.run_sequence(
            [
                ...
                ),  # select/deselect first option
                FlowStep(
                    settings_views.SettingsEntryUpdateSelectionView,
                    screen_return_value=1,
                ),  # select/deselect second option
                FlowStep(
                    settings_views.SettingsEntryUpdateSelectionView,
                    screen_return_value=1,
                ),  # select/deselect second option
    ...
    def test_settingsqr(self):
        ...
        def load_persistent_settingsqr_into_decoder(view: scan_views.ScanView):
            settingsqr_data_persistent: str = (
                "settings::v1 name=Total_noob_mode persistent=E coords=spa,spd denom=thr network=M qr_density=M xpub_export=E sigs=ss scripts=nat xpub_details=E passphrase=E camera=0 compact_seedqr=E bip85=D priv_warn=E dire_warn=E partners=E"
            )
            view.decoder.add_data(settingsqr_data_persistent)

        def load_not_persistent_settingsqr_into_decoder(view: scan_views.ScanView):
            settingsqr_data_not_persistent: str = (
                "settings::v1 name=Ephemeral_noob_mode persistent=D coords=spa,spd denom=thr network=M qr_density=M xpub_export=E sigs=ss scripts=nat xpub_details=E passphrase=E camera=0 compact_seedqr=E bip85=D priv_warn=E dire_warn=E partners=E"
            )
            view.decoder.add_data(settingsqr_data_not_persistent)

        def _run_test(
            initial_setting_state: str,
            load_settingsqr_into_decoder: Callable,
            expected_setting_state: str,
        ):
            self.settings.set_value(
                SettingsConstants.SETTING__PERSISTENT_SETTINGS, initial_setting_state
            )
            self.run_sequence(
                [
                    FlowStep(MainMenuView, button_data_selection=MainMenuView.SCAN),
                    FlowStep(
                        scan_views.ScanView, before_run=load_settingsqr_into_decoder
            ...
            assert (
                self.settings.get_value(SettingsConstants.SETTING__PERSISTENT_SETTINGS)
                == expected_setting_state
            )
        ...
        # DISABLED.
        with patch(
            "seedsigner.models.settings.Settings.HOSTNAME", new_callable=PropertyMock
        ) as mock_hostname:
        ...
        selection_options = SettingsDefinition.get_settings_entry(
            SettingsConstants.SETTING__PERSISTENT_SETTINGS
        ).selection_options
