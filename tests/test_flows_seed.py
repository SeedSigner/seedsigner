...
class TestSeedFlows(FlowTest):
    ...
    def test_electrum_mnemonic_entry_flow(self):
        ...
        """
        def test_with_mnemonic(mnemonic: list[str], expects_electrum_seed_is_valid: bool = True):
            Settings.HOSTNAME = "not seedsigner-os"
        ...
        test_with_mnemonic("gentle combine cool hamster ghost harvest gossip lend dismiss slam any toast".split(), expects_electrum_seed_is_valid=False)
        test_with_mnemonic("enough board blossom stamp fire buffalo digital solution sadness random number stone".split(), expects_electrum_seed_is_valid=False)
