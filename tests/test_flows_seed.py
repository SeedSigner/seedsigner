...
class TestSeedFlows(FlowTest):
    ...
    def test_addressverification_flow(self):
        """
        Scanning an address QR from Home should enter the Address Verification flow,
        including prompting to select or load a seed. Brute-force address verification
        should proceed in a child thread until found.
        """
        ...
        def load_address_into_decoder(view: scan_views.ScanView):
            view.decoder.add_data(target_address)

        def verify_results(view: View):
            assert self.controller.unverified_address["address"] == target_address
            assert self.controller.unverified_address["verified_index"] == expected_index

        self.run_sequence([
            FlowStep(MainMenuView, button_data_selection=MainMenuView.SCAN),
            FlowStep(scan_views.ScanView, before_run=load_address_into_decoder),  # simulate read address QR
            FlowStep(seed_views.AddressVerificationStartView, is_redirect=True),
            FlowStep(seed_views.SeedSelectSeedView, screen_return_value=0),       # ret 1st onboard seed
            FlowStep(seed_views.SeedAddressVerificationView),
            FlowStep(seed_views.AddressVerificationSuccessView, before_run=verify_results),
            FlowStep(MainMenuView)
        ])
