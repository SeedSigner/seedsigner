...
class FlowStep:
    ...
    * is_redirect:           expects the Destination to specify `skip_current_view=True`.
    ...
    def __post_init__(self):
        if (
            self.screen_return_value is not None
            and self.button_data_selection is not None
        ):
            raise Exception(
                "Can't specify both `screen_return_value` and `button_data_selection`"
            )
...
class FlowTest(BaseTest):
    ...
    def run_sequence(
        self, sequence: list[FlowStep], initial_destination_view_args: dict = None
    ) -> None:
        """
        ...
        """
        with patch(
            "seedsigner.views.view.Destination._run_view", autospec=True
        ) as mock_run_view:
            with patch(
                "seedsigner.views.view.View.run_screen", autospec=True
            ) as mock_run_screen:
                ...
                def run_view(destination: Destination, *args, **kwargs):
                    ...
                    if destination.View_cls != cur_flow_step.expected_view:
                        raise FlowTestUnexpectedViewException(
                            f"Expected {cur_flow_step.expected_view}, got {destination.View_cls}"
                        )

                    if len(sequence) == 1:
                        # This is the last step in the sequence
                        if (
                            cur_flow_step.screen_return_value is None
                            and cur_flow_step.button_data_selection is None
                        ):
                    ...
                    try:
                        ...
                        if (
                            mock_run_screen.call_count
                            == prev_mock_run_screen_call_count
                            and cur_flow_step.is_redirect is not True
                        ):
                            # The current View redirected without calling run_screen()
                            # but we weren't expecting it.
                            raise FlowTestUnexpectedRedirectException(
                                f"Unexpected redirect to {destination.View_cls}"
                            )

                        elif (
                            mock_run_screen.call_count > prev_mock_run_screen_call_count
                            and cur_flow_step.is_redirect
                        ):
                            ...
                            raise FlowTestMissingRedirectException(
                                f"FlowStep expected redirect but {cur_flow_step.expected_view} did not redirect"
                            )
                ...
                def run_screen(view: View, *args, **kwargs):
                    ...
                    if "button_data" in kwargs:
                        ...
                        for button_option in kwargs.get("button_data"):
                            if not isinstance(button_option, ButtonOption):
                                raise FlowTestInvalidButtonDataInstanceTypeException(
                                    f"button_data must be a list of ButtonOption instances, not {type(button_option)}: {button_option}"
                                )
                    ...
                    if cur_flow_step.button_data_selection:
                        ...
                        if "button_data" in kwargs:
                            if cur_flow_step.button_data_selection not in kwargs.get(
                                "button_data"
                            ) and cur_flow_step.button_data_selection not in [
                                RET_CODE__BACK_BUTTON,
                                RET_CODE__POWER_BUTTON,
                            ]:
                                raise FlowTestInvalidButtonDataSelectionException(
                                    f"'{cur_flow_step.button_data_selection}' not found in button_data: {kwargs.get('button_data')}"
                                )
                            return kwargs.get("button_data").index(
                                cur_flow_step.button_data_selection
                            )
                        else:
                            raise Exception(
                                f"Can't specify `FlowStep.button_data_selection` if `button_data` isn't a kwarg in {view.__class__.__name__}'s run_screen()"
                            )

                    elif type(cur_flow_step.screen_return_value) in [
                        StopFlowBasedTest,
                ...
                if sequence[0].expected_view != MainMenuView:
                    initial_destination = Destination(
                        sequence[0].expected_view,
                        view_args=initial_destination_view_args,
                    )
                else:
