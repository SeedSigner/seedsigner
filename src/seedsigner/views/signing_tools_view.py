...
class SigningToolsView(View):
    ...
    def display_transaction_information(self, wallet) -> None:
        ...
        self.draw.text(((240 - tw) / 2, 5), "Confirm last 13 chars", fill=View.color, font=View.IMPACT22)
        tw, th = self.draw.textsize("of the receiving address:", font=View.IMPACT22)
        self.draw.text(((240 - tw) / 2, 30), "of the receiving address:", fill=View.color, font=View.IMPACT22)
        tw, th = self.draw.textsize(wallet.destinationaddress[-13:], font=View.IMPACT22)
        self.draw.text(((240 - tw) / 2, 55), wallet.destinationaddress[-13:], fill=View.color, font=View.IMPACT22)
        tw, th = self.draw.textsize("Amount Sending:", font=View.IMPACT22)
        self.draw.text(((240 - tw) / 2, 90), "Amount Sending:", fill=View.color, font=View.IMPACT22)
        if wallet.spend == 0:
            tw, th = self.draw.textsize("Self-Transfer (not parsed)", font=View.IMPACT22)
            self.draw.text(((240 - tw) / 2, 115), "Self-Transfer (not parsed)", fill=View.color, font=View.IMPACT22)
        else:
            tw, th = self.draw.textsize(str(wallet.spend) + " satoshis", font=View.IMPACT22)
            self.draw.text(((240 - tw) / 2, 115), str(wallet.spend) + " satoshis", fill=View.color, font=View.IMPACT22)
        tw, th = self.draw.textsize("Plus a fee of:", font=View.IMPACT22)
        self.draw.text(((240 - tw) / 2, 150), "Plus a fee of:", fill=View.color, font=View.IMPACT22)
        tw, th = self.draw.textsize(str(int(wallet.fee)) + " satoshis", font=View.IMPACT22)
        self.draw.text(((240 - tw) / 2, 175), str(int(wallet.fee)) + " satoshis", fill=View.color, font=View.IMPACT22)
