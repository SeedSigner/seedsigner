...
class SeedsMenuView(View):
    ...
    def run(self):
        ...
        for seed in self.seeds:
            button_data.append(
                ButtonOption(seed["fingerprint"], SeedSignerIconConstants.FINGERPRINT)
            )
...
class SeedSelectSeedView(View):
    ...
    * `flow`: indicates which user flow is in progress during seed selection (e.g.
    ...
    TYPE_ELECTRUM = ButtonOption(
        "Enter Electrum seed", FontAwesomeIconConstants.KEYBOARD
    )
    ...
    def run(self):
        ...
        for seed in seeds:
            ...
            button_data.append(
                ButtonOption(
                    button_str, SeedSignerIconConstants.FINGERPRINT, icon_color="blue"
                )
            )
        ...
        if (
            self.settings.get_value(SettingsConstants.SETTING__ELECTRUM_SEEDS)
...
class SeedAddPassphraseView(View):
    ...
    def __init__(
        self,
        initial_keyboard: str = seed_screens.SeedAddPassphraseScreen.KEYBOARD__LOWERCASE_BUTTON_TEXT,
    ):
        super().__init__()
...
class SeedOptionsView(View):
    ...
    BACKUP = ButtonOption(
        "Backup Seed", right_icon_name=SeedSignerIconConstants.CHEVRON_RIGHT
    )
    ...
    def run(self):
        ...
        if self.controller.resume_main_flow == Controller.FLOW__ADDRESS_EXPLORER:
            ...
            return Destination(
                SeedSignMessageConfirmMessageView, skip_current_view=True
            )
        ...
        if (
            self.settings.get_value(SettingsConstants.SETTING__MESSAGE_SIGNING)
            == SettingsConstants.OPTION__ENABLED
        ):
...
class SeedExportXpubSigTypeView(View):
    ...
    def run(self):
        if len(self.settings.get_value(SettingsConstants.SETTING__SIG_TYPES)) == 1:
            ...
            return Destination(
                ...
                view_args={
                    ...
                    "sig_type": self.settings.get_value(
        ...
        selected_menu_num = self.run_screen(
            ButtonListScreen, title=_("Export Xpub"), button_data=button_data
        )
        ...
        return Destination(
            ...
            view_args={
                "seed_num": self.seed_num,
                "sig_type": button_data[selected_menu_num].return_data,
            },
        )
...
class SeedExportXpubScriptTypeView(View):
    ...
    def run(self):
        ...
        for script_type, display_name in SettingsConstants.ALL_SCRIPT_TYPES:
            if script_type in self.settings.get_value(
...
class SeedExportXpubCoordinatorView(View):
    ...
    def run(self):
        ...
        if len(self.settings.get_value(SettingsConstants.SETTING__COORDINATORS)) == 1:
            # Nothing to select; skip this screen
            args["coordinator"] = self.settings.get_value(
                SettingsConstants.SETTING__COORDINATORS
            )[0]
            return Destination(
                SeedExportXpubWarningView, view_args=args, skip_current_view=True
            )

        button_data = []
        for display_name, setting_option in zip(
            self.settings.get_multiselect_value_display_names(
                SettingsConstants.SETTING__COORDINATORS
            ),
            self.settings.get_value(SettingsConstants.SETTING__COORDINATORS),
        ):
...
class SeedExportXpubWarningView(View):
    ...
    def run(self):
        ...
        if (
            self.settings.get_value(SettingsConstants.SETTING__PRIVACY_WARNINGS)
            == SettingsConstants.OPTION__DISABLED
        ):
...
class SeedExportXpubDetailsView(View):
    ...
    def run(self):
        ...
        if (
            self.settings.get_value(SettingsConstants.SETTING__XPUB_DETAILS)
            == SettingsConstants.OPTION__DISABLED
        ):
            ...
            try:
                ...
                embit_network = NETWORKS[
                    SettingsConstants.map_network_to_embit(
                        self.settings.get_value(SettingsConstants.SETTING__NETWORK)
                    )
                ]
                ...
                root = HDKey.from_seed(
                    self.seed.seed_bytes, version=embit_network["xprv"]
                )
...
class SeedWordsWarningView(View):
    ...
    def run(self):
        ...
        selected_menu_num = self.run_screen(
            DireWarningScreen,
            text=_(
                "You must keep your seed words private & away from all online devices."
            ),
        )
    ...
    *  XPRV (BIP32)
    ...
    def run(self):
        ...
        selected_menu_num = ButtonListScreen(
            title=_("BIP-85 Num Words"), button_data=button_data
        ).display()
...
class SeedWordsBackupTestView(View):
    ...
    def run(self):
        ...
        if self.rand_seed is not None:
            random.seed(
                self.rand_seed + self.cur_index if self.cur_index is not None else 0
            )
...
class SeedTranscribeSeedQRConfirmScanView(View):
    ...
    def run(self):
        ...
        self.run_screen(
            ScanScreen, decoder=self.decoder, instructions_text=_("Scan your SeedQR")
        )
        ...
        if self.decoder.is_complete:
            if self.decoder.is_seed:
                ...
                if seed_mnemonic != self.seed.mnemonic_list:
                    return Destination(
                        SeedTranscribeSeedQRConfirmWrongSeedView, skip_current_view=True
                    )
                else:
                    return Destination(
            ...
            return Destination(
                SeedTranscribeSeedQRConfirmInvalidQRView, skip_current_view=True
            )
...
class AddressVerificationStartView(View):
    ...
    def run(self):
        ...
        if (
            ...
            destination = Destination(
                SeedSelectSeedView,
                view_args=dict(flow=Controller.FLOW__VERIFY_SINGLESIG_ADDR),
                skip_current_view=True,
            )
...
class AddressVerificationSigTypeView(View):
    ...
    def run(self):
        ...
        selected_menu_num = self.run_screen(
            ...
            title=_("Verify Address"),
            text=_(
                "Sig type can't be auto-detected from this address. Please specify:"
            ),
            button_data=button_data,
            is_bottom_list=True,
        )
...
class SeedAddressVerificationView(View):
    ...
    def __init__(self, seed_num: int = None):
        ...
        if not self.is_multisig:
            if seed_num is None:
                raise Exception(
                    _("Can't validate a single sig addr without specifying a seed")
                )
            self.seed_num = seed_num
            self.seed = self.controller.get_seed(seed_num)
            self.seed_derivation_override = self.seed.derivation_override(
                sig_type=SettingsConstants.SINGLE_SIG
            )
        else:
            self.seed = None
        ...
        self.derivation_path = (
            self.seed_derivation_override
            if self.seed_derivation_override
            else self.controller.unverified_address["derivation_path"]
        )
    ...
    def run(self):
        ...
        try:
            ...
            if self.verified_index.cur_count is not None:
                ...
                self.controller.unverified_address["verified_index"] = (
...
class MultisigWalletDescriptorView(View):
    ...
    def run(self):
        ...
        elif button_data[selected_menu_num].button_label.startswith(
            _(self.VERIFY_ADDR.button_label)
        ):
            self.controller.resume_main_flow = None
...
class SeedSignMessageStartView(View):
    ...
    def __init__(self, derivation_path: str, message: str):
        ...
        if not addr_format["clean_match"]:
            self.set_redirect(
                Destination(
                    NotYetImplementedView,
                    view_args=dict(
                        text=f"Signing messages for custom derivation paths not supported"
                    ),
                )
            )
            self.controller.resume_main_flow = None
        ...
        if self.seed_num is not None:
            ...
            self.set_redirect(
                Destination(SeedSignMessageConfirmMessageView, skip_current_view=True)
            )
        else:
            from seedsigner.controller import Controller
    ...
    def run(self):
        ...
        if self.page_num == len(self.controller.sign_message_data["paged_message"]) - 1:
            ...
            return Destination(
                ...
                view_args=dict(page_num=self.page_num + 1),
            )
...
class SeedSignMessageConfirmAddressView(View):
    def __init__(self):
        ...
        if (
            not addr_format["clean_match"]
            or addr_format["script_type"] == SettingsConstants.CUSTOM_DERIVATION
        ):
            raise Exception(
                _("Signing messages for custom derivation paths not supported")
            )
        ...
        if addr_format["network"] != SettingsConstants.MAINNET:
            ...
            if self.settings.get_value(SettingsConstants.SETTING__NETWORK) in [
                ...
                addr_format["network"] = self.settings.get_value(
                    SettingsConstants.SETTING__NETWORK
                )
            else:
                from seedsigner.views.view import NetworkMismatchErrorView
        ...
        xpub = seed.get_xpub(
            wallet_path=addr_format["wallet_derivation_path"],
            network=addr_format["network"],
        )
...
class SeedSignMessageSignedMessageQRView(View):
    ...
    def __init__(self):
        ...
        self.signed_message = embit_utils.sign_message(
            seed_bytes=seed.seed_bytes, derivation=derivation_path, msg=message.encode()
        )
