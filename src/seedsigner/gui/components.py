...
class TextArea(BaseComponent):
    ...
    def __post_init__(self):
        ...
        if len(self.text_lines) == 1:
            ...
            if not self.height_ignores_below_baseline and re.findall(
                f"[gjpqy]", self.text_lines[-1]["text"]
            ):
        ...
        if self.supersampling_factor > 1:
            resized = img.resize(
                (image_width, total_text_height + 2 * resample_padding), Image.LANCZOS
            )
