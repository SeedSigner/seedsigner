    ...
    def _run(self):
        ...
        while True:
            ...
            if self.hw_inputs.check_for_low(
                keys=HardwareButtonsConstants.KEYS__ANYCLICK
            ):
                # Have to manually update last input time since we're not in a wait_for loop
                self.hw_inputs.update_last_input_time()
                self.camera.stop_video_stream_mode()
            ...
            with self.renderer.lock:
                ...
                self.renderer.draw.text(
                    ...
                    + _(
                        "click a button"
                    ),  # TODO: Render with UI elements instead of text
...
class ToolsDiceEntropyEntryScreen(KeyboardScreen):
    ...
    def update_title(self) -> bool:
        self.title = _("Dice Roll {}/{}").format(
            self.cursor_position + 1, self.return_after_n_chars
        )
        return True
            ...
            if self.script_type != SettingsConstants.CUSTOM_DERIVATION:
                self.components.append(
                    IconTextLine(
                        ...
                        value_text=SettingsDefinition.get_settings_entry(
