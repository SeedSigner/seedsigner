...
class PSBTOverviewScreen(ButtonListScreen):
    ...
    def __post_init__(self):
            ...
            if len(self.destination_addresses) + self.num_self_transfer_outputs <= 3:
                ...
                for i in range(0, self.num_self_transfer_outputs):
                    destination_column.append(
                        truncate_destination_addr("self-transfer")
                    )
            else:
                ...
                destination_column.append(
                    f"recipient {len(self.destination_addresses) + self.num_self_transfer_outputs}"
                )
        ...
        if len(self.destination_addresses) + self.num_self_transfer_outputs > 3:
            ...
            for i in range(6, 14):
                (new_width, new_col_text) = calculate_destination_col_width(
                    truncate_at=i
                )
...
class PSBTMathScreen(ButtonListScreen):
    ...
    def __post_init__(self):
        ...
        fixed_width_font = Fonts.get_font(
            GUIConstants.FIXED_WIDTH_FONT_NAME, (GUIConstants.BODY_FONT_SIZE + 6) * ssf
        )
        ...
        def render_amount(
            ...
            draw.text(
                (digits_width + 2 * digit_group_spacing, cur_y),
                text=info_text,
                font=body_font,
