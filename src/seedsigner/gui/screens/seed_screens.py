...
class SeedMnemonicEntryScreen(BaseTopNavScreen):
    ...
    def __post_init__(self):
        ...
        self.word_font = Fonts.get_font(
            ...
            GUIConstants.get_button_font_size() + 4,
        )
...
class SeedWordsScreen(WarningEdgesMixin, ButtonListScreen):
    ...
    def __post_init__(self):
        ...
        font = Fonts.get_font(
            ...
            (GUIConstants.get_top_nav_title_font_size() + 2) * supersampling_factor,
        )
        ...
        number_font = Fonts.get_font(
            GUIConstants.get_body_font_name(),
            GUIConstants.get_button_font_size() * supersampling_factor,
        )
    ...
    def __post_init__(self):
        ...
        self.components.append(
            IconTextLine(
                ...
                - GUIConstants.COMPONENT_PADDING
                - int(GUIConstants.get_body_font_size() * 2.5),
            )
        )
...
class SeedTranscribeSeedQRZoomedInScreen(BaseScreen):
    ...
    def __post_init__(self):
        ...
        draw.text(
            (
                ...
                self.canvas_height - int(GUIConstants.COMPONENT_PADDING / 2),
            ),
            msg,
            fill=GUIConstants.BODY_FONT_COLOR,
            font=font,
    ...
    def _render(self):
        ...
        self.cur_x = (
            (self.cur_block_x * self.qr_blocks_per_zoom * self.pixels_per_block)
            + self.qr_border * self.pixels_per_block
            - self.mask_width
        )
        self.cur_y = (
            (self.cur_block_y * self.qr_blocks_per_zoom * self.pixels_per_block)
            + self.qr_border * self.pixels_per_block
            - self.mask_height
        )
        ...
        self.renderer.show_image(
            self.qr_image.crop(
                (
                    self.cur_x,
                    self.cur_y,
                    self.cur_x + self.canvas_width,
                    self.cur_y + self.canvas_height,
                )
            ),
    ...
    def _run(self):
        while True:
            ...
            if input == HardwareButtonsConstants.KEY_RIGHT:
                self.next_x = (
                    self.cur_x + self.qr_blocks_per_zoom * self.pixels_per_block
                )
                self.cur_block_x += 1
                ...
                self.next_y = (
                    self.cur_y + self.qr_blocks_per_zoom * self.pixels_per_block
                )
                self.cur_block_y += 1
            ...
            if self.cur_x != self.next_x or self.cur_y != self.next_y:
                with self.renderer.lock:
                    self.renderer.show_image_pan(
                        ...
                        alpha_overlay=Image.alpha_composite(
                            self.block_mask, block_labels
                        ),
                    )
        ...
        def run(self):
            while self.keep_running:
                ...
                textarea = TextArea(
                    ...
                    text=_("Checking address {}").format(
                        self.threadsafe_counter.cur_count
                    ),
...
class LoadMultisigWalletDescriptorScreen(ButtonListScreen):
    def __post_init__(self):
        ...
        self.components.append(
            TextArea(
                text=_(
                    "Load your multisig wallet descriptor to verify your receive/self-transfer or change address."
                ),
                screen_y=self.top_nav.height,
                height=self.buttons[0].screen_y - self.top_nav.height,
            )
...
class SeedSignMessageConfirmAddressScreen(ButtonListScreen):
    ...
    def __post_init__(self):
        ...
        address_display = FormattedAddress(
            address=self.address,
            max_lines=3,
            screen_y=derivation_path_display.screen_y
            + derivation_path_display.height
            + 2 * GUIConstants.COMPONENT_PADDING,
        )
        self.components.append(address_display)
