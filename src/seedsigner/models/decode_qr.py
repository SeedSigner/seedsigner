...
class SettingsQrDecoder(BaseSingleFrameQrDecoder):
    ...
    def add(self, segment, qr_type=QRType.SETTINGS):
        ...
        try:
            ...
            def convert_abbreviated_value(
                category, key, abbreviation_map, is_list=False, new_key_name=None
            ):
                try:
                    ...
                    if not is_list:
                        ...
                        if not new_value:
                            logger.error(
                                f'No abbreviation map value for "{value}" for setting {key}'
                            )
                            return
                    else:
                        ...
                        for v in values:
                            ...
                            if not mapped_value:
                                logger.error(
                                    f'No abbreviation map value for "{v}" for setting {key}'
                                )
                                return
            ...
            convert_abbreviated_value(
                "wallet",
                "coord",
                map_abbreviated_coordinators,
                is_list=True,
                new_key_name="coordinators",
            )
            convert_abbreviated_value(
                "features", "xpub", map_abbreviated_enable, new_key_name="xpub_export"
            )
            convert_abbreviated_value(
                "features",
                "sigs",
            ...
            convert_abbreviated_value(
                ...
                map_abbreviated_scripts,
                is_list=True,
                new_key_name="script_types",
            )
            convert_abbreviated_value(
                "features",
                "xp_det",
                map_abbreviated_enable,
                new_key_name="show_xpub_details",
            )
            ...
            convert_abbreviated_value(
                ...
                "priv_warn",
                map_abbreviated_enable,
                new_key_name="show_privacy_warnings",
            )
            convert_abbreviated_value(
                "features",
                "dire_warn",
                map_abbreviated_enable,
                new_key_name="show_dire_warnings",
            )
